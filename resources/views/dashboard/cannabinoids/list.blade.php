                ...
                <table class="table mb-0">
                    ...
                    <tbody>
                        @foreach($cannabinoids as $cannabinoid)
                            <tr>
                                <td>{{ $cannabinoid->id }}</td>
                                <td>
                                    @if($cannabinoid->icon)
                                        <img src="{{ $cannabinoid->icon }}" alt="{{ $cannabinoid->title }}">
                                    @else
                                        <div class="badge bg-warning text-dark">немає</div>
                                    @endif
                                </td>
                                <td>{{ $cannabinoid->title }}</td>
                                <td>
                                    @if($cannabinoid->description)
                                        <div class="badge bg-success">заповнено</div>
                                    @else
                                        <div class="badge bg-warning text-dark">немає</div>
                                    @endif
                                </td>
                                <td>{{ $cannabinoid->slug }}</td>
                                <td>
                                    @if($cannabinoid->created_at)
                                        {{ $cannabinoid->created_at->format('d/m/Y') }}
                                    @else
                                        --
                                    @endif
                                </td>
                                <td>
                                    @if($cannabinoid->active)
                                ...
                                <td align="right">
                                    <a href="{{ route('dashboard.cannabinoid.edit', $cannabinoid->id) }}" class="btn btn-primary btn-sm">
